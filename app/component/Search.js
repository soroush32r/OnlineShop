...
const Search = () => {
  ...
  return (
    <div className="text-black relative">
      <input placeholder="search" value={search} onChange={handleChange} className="border-none rounded px-2 py-1"/>
      <div className="w-full absolute shadow-md">
        {!!search && SEARCH_PRODUCT.map(product => (
          <Link href={`/${product.category}/${product.name}`} onClick={() => setSearch('')}>
